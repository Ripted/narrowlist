...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (fetchError) {
      throw new Error(`Failed to fetch future levels: ${fetchError.message}`);
    }

    if (!levels || levels.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: "No future levels to resync", updated: 0 }),
      ...
      try {
        ...
        if (Object.keys(updates).length > 0) {
          ...
          if (updateError) {
            ...
            updatedCount++;
            console.log(`Updated future level ${level.level_id}: ${JSON.stringify(updates)}`);
          }
        }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Resync complete. Updated ${updatedCount} of ${levels.length} future levels.`,
