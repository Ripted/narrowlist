...
export function BulkTagAssigner({ onClose }: BulkTagAssignerProps) {
  ...
  const applyTags = async () => {
    ...
    if (errorCount > 0) {
      toast({ 
        ...
        description: `Applied to ${successCount} levels, ${skipCount} skipped, ${errorCount} failed`,
        variant: "destructive" 
      });
    } else {
      toast({ 
        title: "Success", 
        description: `Tag applied to ${successCount} levels${skipCount > 0 ? `, ${skipCount} already had it` : ""}` 
      });
      setSelectedPreset("");
      setSelectedLevelIds(new Set());
    }
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-border rounded-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-hidden flex flex-col p-4 gap-4">
          ...
          {presetsLoading ? (
            ...
            <div className="rounded-lg border border-dashed border-border p-4 text-center">
              <p className="text-sm text-muted-foreground">No tag presets available.</p>
              <p className="text-xs text-muted-foreground mt-1">Create presets in the Tag Presets Manager first.</p>
            </div>
          ) : (
            <div className="rounded-lg border border-dashed border-border p-4 space-y-3">
              <Label className="text-sm font-medium">Select Tag Preset:</Label>
              ...
              {selectedPresetData && (
                <div className="flex items-center gap-2 p-2 bg-secondary/50 rounded-lg">
                  <span className="text-xl">{selectedPresetData.emoji}</span>
                  <span className="font-medium">{selectedPresetData.text}</span>
                  {selectedPresetData.description && (
                    <span className="text-xs text-muted-foreground ml-2">{selectedPresetData.description}</span>
                  )}
                </div>
              )}
        ...
        <div className="p-4 border-t border-border flex justify-end gap-2">
          ...
          <Button
            onClick={applyTags}
            disabled={applying || selectedLevelIds.size === 0 || !selectedPreset || presets.length === 0}
            className="gap-2"
          >
            {applying ? (
              <>
              ...
              <>
                ...
                Assign Tag to {selectedLevelIds.size} Levels
              </>
            )}
          </Button>
        </div>
