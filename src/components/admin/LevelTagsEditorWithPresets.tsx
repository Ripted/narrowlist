...
interface LevelTagsEditorWithPresetsProps {
  levelId: string;
  levelType: "main" | "future" | "extra";
  onClose?: () => void;
}

export function LevelTagsEditorWithPresets({ levelId, levelType, onClose }: LevelTagsEditorWithPresetsProps) {
  const { toast } = useToast();
  const { presets } = useTagPresets();
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Add New Tag - with preset selection */}
      <div className="border border-dashed border-border rounded-lg p-3 space-y-3">
        {presets.length > 0 && (
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Quick add from preset</label>
